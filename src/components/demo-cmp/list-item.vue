<template>
  ...
  >
    ...
    <div :class="`font(${fontName} bold) italic`">{{ name }} bold italic: {{ t('welcome') }}</div>
    <div :class="`font(${fontName} bold)`">{{ name }} bold: {{ t('welcome') }}</div>
  </div>
</template>
...
const isRenderSuccessfully = (name: string): boolean => {
  if (name.length > 0) {
    ...
    throw Error('Property name is empty');
  }
};
</script>
